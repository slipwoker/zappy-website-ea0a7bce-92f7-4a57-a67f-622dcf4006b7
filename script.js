document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        function showError(input, message) {
            var error = createErrorElement(input);
            error.textContent = message;
            input.style.borderColor = '#e74c3c';
        }
        
        function validateField(input) {
            var fieldConfig = fields.find(function(f) { return f.name === input.name; });
            ...
            if (fieldConfig.minLength && value.length < fieldConfig.minLength) {
                showError(input, fieldConfig.label + ' must be at least ' + fieldConfig.minLength + ' characters');
                return false;
            }
            
            if (fieldConfig.pattern && !fieldConfig.pattern.test(value)) {
                showError(input, 'Please enter a valid ' + fieldConfig.label.toLowerCase());
                return false;
            }
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            fields.forEach(function(field) {
                var input = contactForm.querySelector('[name="' + field.name + '"]');
                if (input) {
        ...
        contactForm.addEventListener('input', function(e) {
            var input = e.target;
            if (input.name && fields.some(function(f) { return f.name === input.name; })) {
                if (input.value.trim()) {
                    validateField(input);
                }
            }
    ...
    if (animatedElements.length > 0) {
        ...
        style.textContent = '.fade-in.visible,.fade-in-up.visible,.fade-in-left.visible,.fade-in-right.visible{opacity:1!important;transform:translate(0,0)!important}';
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'ea0a7bce-92f7-4a57-a67f-622dcf4006b7',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'ea0a7bce-92f7-4a57-a67f-622dcf4006b7';
...
/* ZAPPY_CUSTOM_JS_START:5bec5da828f3 */
(function () {
  function __zappyCustomInit() {
    try {
(function() {
  // Intercept category links in nav sub-menu to set sessionStorage before navigation
  function bindCategoryLinks() {
    var subMenuLinks = document.querySelectorAll('.nav-menu .sub-menu a');
    subMenuLinks.forEach(function(link) {
      var text = link.textContent.trim();
      var cat = '';
      if (text === 'דיני משפחה') cat = 'דיני משפחה';
      else if (text === 'דינים אישיים') cat = 'דינים אישיים';
      else if (text === 'תביעות נכי צה"ל/ביטוח לאומי') cat = 'תביעות נכי צה"ל/ביטוח לאומי';
      if (cat) {
        link.addEventListener('click', function(e) {
          sessionStorage.setItem('articlesCategory', cat);
        });
      }
    });
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bindCategoryLinks);
  } else {
    bindCategoryLinks();
  }
})();
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'ea0a7bce-92f7-4a57-a67f-622dcf4006b7';
